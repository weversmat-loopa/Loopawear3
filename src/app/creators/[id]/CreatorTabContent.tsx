...
interface CreatorTabContentProps {
  /** Initial active tab (from URL searchParam on first load) */
  initialTab: TabId;
  profileId: string;
...
export default function CreatorTabContent({
  ...
  function handleTabClick(tabId: TabId) {
    setActiveTab(tabId);
    // Keep the URL in sync (for deep-linking / sharing) without triggering a
    // server re-render. replace: true so the tab switches don't stack in history.
    router.replace(`/creators/${profileId}?tab=${tabId}`, { scroll: false });
  }
  ...
  return (
    <div className="mt-10">
      ...
      {/* ══════════════════════════════════════════════════════════
          TAB: DRAFTS  (owner only)
          ownerData is only present when isOwnProfile === true,
          so this block never renders for public visitors.
      ══════════════════════════════════════════════════════════ */}
